...
class _DragBar(QtWidgets.QWidget):
    ...
    def mousePressEvent(self, e: QtGui.QMouseEvent) -> None:
        self._overlay.start_drag(e)

    def mouseMoveEvent(self, e: QtGui.QMouseEvent) -> None:
        self._overlay.do_drag(e)

    def mouseReleaseEvent(self, e: QtGui.QMouseEvent) -> None:
        self._overlay.end_drag(e)
...
class OverlayWidget(QtWidgets.QWidget):
    ...
    def apply_theme(self, mode: str) -> None:
        self._tokens = THEMES.get(mode, THEMES["light"])
        self._apply_theme()
    ...
    # ── drag handling ───────────────────────────────────────────────────────────
    def start_drag(self, e: QtGui.QMouseEvent) -> None:
    ...
    def do_drag(self, e: QtGui.QMouseEvent) -> None:
        if self._dragging and e.buttons() & QtCore.Qt.MouseButton.LeftButton:
            self.move(e.globalPosition().toPoint() - self._drag_pos)

    def end_drag(self, e: QtGui.QMouseEvent) -> None:
        if e.button() == QtCore.Qt.MouseButton.LeftButton:
            self._dragging = False
...
class OverlayManager:
    def __init__(self, events: EventEmitter | None = None) -> None:
        ...
        self._events = events
        self._signals = type("S", (QtCore.QObject,) , {"tick": QtCore.pyqtSignal(int, int)})()
        self._signals.tick.connect(lambda r, e: self._widget.set_remaining(r, e))

        self._widget.paused.connect(lambda:  events.emit("timer:pause", {}))
        self._widget.resumed.connect(lambda:  events.emit("timer:resume", {}))
        self._widget.stopped.connect(lambda:  events.emit("timer:stop", {}))
        ...
        if events:
            ...
            events.on("timer:resume", lambda _: self._widget.start_timer())
